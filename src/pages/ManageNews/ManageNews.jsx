...
//       endpoint = `search?q=${encodeURIComponent(search)}&page=${page}&per_page=${perPage}`;
//       response = await httpClient.get(endpoint);
      
//       if (response.data.success) {
//         // Extract news data from search response
//         return { 
//           articles: response.data.data.news.data,
//           pagination: response.data.data.news.pagination 
//         };
//       }
//     } else {
...
//   // Fetch articles when tab, pagination, or search term changes
//   const fetchArticles = useCallback(async (newPage = pagination?.current_page || 1, newPerPage = pagination?.per_page || 10, search = searchTerm) => {
//     setIsLoading(true);
//     try {
//       let endpoint;
//       let response;
...
//       if (search.trim()) {
//         // Use search endpoint when search term is provided
//         endpoint = `search?q=${encodeURIComponent(search)}&page=${newPage}&per_page=${newPerPage}`;
//         response = await httpClient.get(endpoint);
        
//         if (response.data.success) {
//           // Extract news data from search response
//           setArticles(response.data.data.news.data || []);
//           setPagination(response.data.data.news.pagination || {
//             total: 0,
//             current_page: 1,
//             last_page: 1,
//             per_page: 10
...
//       setIsLoading(false);
//     }
//   }, [tab, searchTerm, pagination?.current_page, pagination?.per_page]);
...
//   // Fetch articles when tab, page, perPage, or search params change
//   useEffect(() => {
//     const currentPage = parseInt(searchParams.get("page")) || 1;
//     const currentPerPage = parseInt(searchParams.get("perPage")) || 10;
...
//     fetchArticles(currentPage, currentPerPage, currentSearch);
//   }, [tab, searchParams, fetchArticles]);
