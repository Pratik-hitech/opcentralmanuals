...
//     try {
//       // Create FormData only if we have files to upload
//       let payload;
//       let config = {};
      
//       if (imageFiles.primary || imageFiles.featured) {
//         // Use FormData if we have files
//         payload = new FormData();
//         payload.append("title", formData.title);
//         payload.append("slug", formData.slug);
//         payload.append("content", formData.content);
//         payload.append("pinned", formData.pinned ? 1 : 0);
//         payload.append("featured", formData.featured ? 1 : 0);
//         payload.append("status", formData.schedule ? "draft" : formData.status);
//         payload.append("company_id", user.company_id);
...
//         // Append images if they are new files
//         if (imageFiles.primary) {
//           payload.append("primary_image", imageFiles.primary);
//         }
        
//         if (imageFiles.featured) {
//           payload.append("featured_image", imageFiles.featured);
//         }
        
//         // Append videos
//         formData.videos.forEach((video, index) => {
//           payload.append(`videos[${index}][type]`, video.type);
//           payload.append(`videos[${index}][title]`, video.title);
//           payload.append(`videos[${index}][reference_url]`, video.reference_url || "");
//         });
...
//       const response = isEditMode
//         ? await httpClient.put(`news/${id}`, payload, config)
//         : await httpClient.post("news", payload, config);
...
//                     <Box sx={{ pr: 2, overflow: "hidden" }}>
//                       <Typography variant="subtitle2" noWrap title={v.title}>{v.title}</Typography>
//                       <Typography variant="caption" color="text.secondary">{v.type.toUpperCase()} • {v.reference_url || "-"}</Typography>
//                     </Box>
//                     <IconButton onClick={() => handleDeleteVideo(idx)} color="error" size="small">
//                       <DeleteIcon fontSize="small" />
//                     </IconButton>
