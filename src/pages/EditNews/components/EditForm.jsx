...
//   const newAttachments = files.map(file => ({
//     file,                               // File object for FormData upload
//     name: file.name,                     // Original file name
//     size: file.size,
//     type: file.type,
//     url: URL.createObjectURL(file),      // Preview URL
//     isNew: true                          // Mark as new attachment
//   }));
...
//   if (attachment.isNew) {
//     // New attachment (not yet uploaded) → just remove locally
//     setFormData(prev => ({
//       ...prev,
//       attachments: prev.attachments.filter((_, i) => i !== idx),
//     }));
...
const NewsArticleForm = () => {
  ...
  const [videoModalOpen, setVideoModalOpen] = useState(false);
  const [isScheduled, setIsScheduled] = useState(false);
  const [videosToDelete, setVideosToDelete] = useState([]); // Track videos to be deleted
  const [attachmentsToDelete, setAttachmentsToDelete] = useState([]); // Track attachments to be deleted
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const hasSchedule = Boolean(data.schedule_at);
          setIsScheduled(hasSchedule);
          ...
          setFormData(prev => ({
            ...
            featured_image: data.featured_image || null,
            status: data.status || "DRAFT",
            schedule_at: formatDateForInput(data.schedule_at || data.schedule),
            videos: normalizedVideos,
            attachments: processedAttachments,
          }));
  ...
  const handleDeleteVideo = (idx) => {
    const videoToDelete = formData.videos[idx];
    
    // If it's an existing video (has an ID), mark it for deletion
    if (videoToDelete.id) {
      setVideosToDelete(prev => [...prev, videoToDelete.id]);
    }
  ...
  const handleDeleteAttachment = async (idx) => {
    ...
    if (attachment.isNew) {
      ...
      setAttachmentsToDelete(prev => [...prev, attachment.id]);
      setFormData(prev => ({
        ...prev,
        attachments: prev.attachments.filter((_, i) => i !== idx),
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Set status based on user selection (not forced to ARCHIVED when scheduled)
      if (isScheduled) {
        formDataObj.append("status", formData.status);
        formDataObj.append("schedule_at", formattedSchedule);
      } else {
      ...
      // Handle videos - send null if all videos are to be deleted
      if (formData.videos.length === 0 && videosToDelete.length > 0) {
        // All videos are marked for deletion
        formDataObj.append("videos", "");
      ...
      // Handle attachments with proper payload structure
      if (formData.attachments.length === 0 && attachmentsToDelete.length > 0) {
        // All attachments are marked for deletion
        formDataObj.append("attachments", "");
      } else if (formData.attachments.length > 0) {
        // Append attachments with proper structure
        formData.attachments.forEach((attachment, index) => {
          if (attachment.isNew) {
            // New attachments - append the file directly
            formDataObj.append(`attachments[${index}]`, attachment.file);
          } else {
            // Existing attachments - use the proper structure with id and reference_url
            formDataObj.append(`attachments[${index}][id]`, attachment.id);
            formDataObj.append(`attachments[${index}][reference_url]`, attachment.url);
          }
        });
      }
    ...
    <Box component="form" onSubmit={handleSubmit} sx={{ maxWidth: '70%', margin: '0 auto', '@media (max-width: 900px)': { maxWidth: '90%' } }}>
      ...
      <Paper elevation={3} sx={{ p: 3 }}>
        ...
        <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
          <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1.5 }}>
            <Typography variant="subtitle1">Attachments</Typography>
            <Button variant="outlaid" component="label" startIcon={<AddIcon />}>
              Add Attachment
              <input type="file" hidden multiple onChange={handleAttachmentSelect} />
            </Button>
          </Box>
        ...
        {/* Schedule Switch */}
          ...
          {formData.status === "ARCHIVED" && (
            <Typography variant="caption" color="text.secondary" sx={{ ml: 2 }}>
              Cannot schedule archived articles
            </Typography>
          )}
        </Box>
        ...
        {/* Status - Always show status selection regardless of schedule */}
         <FormControl fullWidth sx={{ mb: 3 }}>
          <InputLabel>Status</InputLabel>
          <Select
            value={formData.status}
            onChange={(e) => {
              ...
              // Automatically turn off scheduling when status is ARCHIVED
